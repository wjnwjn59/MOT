...
Updated for prediction folder structure like:

output/test/tracking_results/hiptrack/hiptrack_cls_0.1/got10k/
    1-Boat.txt
    1-Boat_time.txt
    2-ship.txt
    2-ship_time.txt
...
def read_bounding_boxes(file_path):
    ...
    Format: x,y,width,height per line (comma or tab/space separated)
    ...
    if os.path.exists(file_path):
        with open(file_path, 'r') as f:
            for line in f:
                line = line.strip()
                if line:
                    # Try comma first, then tab/space
                    if ',' in line:
...
def process_sequence(dataset_path, predictions_path, sequence_name, output_dir):
    ...
    Args:
        ...
        predictions_path: path to predictions directory (contains *.txt files)
        sequence_name: name of the sequence (e.g., '1-Boat')
    ...
    # Prediction file is directly under predictions_path (no per-sequence subfolder)
    pred_boxes = []
    pred_file = os.path.join(predictions_path, f"{sequence_name}.txt")
    if os.path.exists(pred_file):
        pred_boxes = read_bounding_boxes(pred_file)
        print(f"Using prediction file: {os.path.basename(pred_file)}")
    else:
        # Optional fallback to *_001.txt if needed
        pred_file = os.path.join(predictions_path, f"{sequence_name}_001.txt")
        if os.path.exists(pred_file):
            pred_boxes = read_bounding_boxes(pred_file)
            print(f"Using prediction file: {os.path.basename(pred_file)}")
        else:
...
def main():
    ...
    parser.add_argument("--dataset_path", type=str, default="../data/test",
                        help="Path to dataset directory (contains per-sequence folders)")
    parser.add_argument("--predictions_path", type=str, 
                        default="./output/test/tracking_results/hiptrack/hiptrack_cls_0.1/got10k",
                        help="Path to predictions directory (contains *.txt files)")
    ...
    parser.add_argument("--sequences", type=str, nargs="+", default=None,
                        help="Specific sequences to process (e.g., 1-Boat 2-ship). "
                             "If not specified, infer from prediction files.")
    ...
    if args.sequences:
        ...
        # Infer sequences from prediction files: *.txt excluding *_time.txt
        txt_files = glob.glob(os.path.join(args.predictions_path, "*.txt"))
        sequences = []
        for fpath in txt_files:
            base = os.path.basename(fpath)
            if base.endswith("_time.txt"):
                continue
            seq_name = os.path.splitext(base)[0]
