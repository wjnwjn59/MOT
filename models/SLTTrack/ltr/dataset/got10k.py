...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        if split is not None:
            ...
            seq_names = pandas.read_csv(file_path, header=None, squeeze=True).values.tolist()
            seq_ids = [self.sequence_list.index(name) for name in seq_names if name in self.sequence_list]
