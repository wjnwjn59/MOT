...
class CLIPVIT(nn.Module):
    ...
    def __init__(self, num_features=768, template_sz=112, search_sz=224, window_sz=16, foveal_sz=64, net_name='ViT-B/16', **kwargs):
        ...
        with torch.no_grad():
            ...
            x2 = x1.clone() + self.wz
            # Change .cuda() to .cpu() for CPU compatibility
            self.all_anchor = torch.stack([x1, y1, x2, y2], 1).unsqueeze(0) / (self.wz * self.tz)
            self.all_pos = torch.stack([(x1 + x2) / 2, (y1 + y2) / 2], 1).unsqueeze(0) / (self.wz * self.tz)
    ...
    def forward(self, input):
        ...
        cur_anchor = self.all_anchor.to(x0.device).repeat(bz_, 1, 1).reshape(-1, 4)  # Move to same device
        ...
        if self.shift_flag:
            xts = torch.zeros(xt0.size()).to(xt0.device)  # Use same device as input
        ...
        all_pos = self.all_pos.to(x0.device).repeat(bz_, 1, 1).reshape(-1, 2)  # Move to same device
